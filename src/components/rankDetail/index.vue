...
"></use>
     ...
     <!-- 头像 -->
     <div class='avator' 
     :style="{'background-image':`url(${myimgs})`}" 
     ref='img'
     >
...
<script>
import {getSongUrlByMid,getRankSong} from '../../api/api'
...
export default {
  ...
  async created(){
        ...
        let data = await getRankSong(id)
        // console.log(ele)
        // let data = JSON.parse(ele.slice(5,-1))
        
        this.title = data.topinfo.ListName
        // this.myimgs = data.cdlist[0].logo
